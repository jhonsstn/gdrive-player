...
export function compareNaturalName(left: string, right: string, direction: SortDirection): number {
...
export function sortByNaturalName<T extends { name: string }>(
  ...
  return [...items].sort((left, right) => compareNaturalName(left.name, right.name, direction));
}
